...
#Count the Refs
pat1 = re.compile("\<ref.*?\>.*?\<\/ref\>",re.DOTALL)
#Exclude the commented out ones?
...
def exclusions(page):
  ...
  page_text = page.get()
  #Count the Refs
  m1 = pat1.findall(page_text)
  #Exclude the commented out ones?
  m2 = pat2.findall(page_text)
  commented_out = 0
  for i in m2:
    commented_out += len(pat1.findall(i))
  c1 = (len(m1) - commented_out)> 1
  if c1:
    return False
  if '{{GR' in page_text:
    return False
  if '{{sfn' in page_text:
    return False
  if 'National Register of Historic Places listings in' in page.title():
    return False
  return True
  

def main(*args):
    logger = logging.getLogger('translusion')
    logger.setLevel(logging.INFO)
    hand = logging.handlers.TimedRotatingFileHandler('logs/nrsi_list', \
        when='D', \
        interval = 1, \
        backupCount = 90, \
    )
        ...
        for arg in pywikibot.handleArgs(*args):
            ...
            if gen:
                ...
                for page in my_list:
                    if not enable:
                      continue
                    logger.info("List: %s" % page.title())
                    if exclusions(page):
                      i = i + 1
                      result_str = result_str + "#[[%s]]\n" % page.title()
                      logger.info("Matched: %s" % page.title())
                      result_page = pywikibot.Page(
                        pywikibot.getSite(),
                        'User:HasteurBot/NRISref'
                      )
                      result_str = result_page.get() +'\n' + result_str
                      result_page.put(result_str)
                      result_str = ''
                    if page.title() == 'Beaman and Smith Company Mill':
                      enable = True
